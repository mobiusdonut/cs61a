...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        roll = dice()
        roll_sum += roll
        sow_sad = sow_sad or (roll == 1)
    return 1 if sow_sad else roll_sum
...
def num_factors(n):
    ...
    while factor > int(n ** 0.5):
        if n % factor == 0:
            count_factors += 1
        factor -= 1
    return 2 * count_factors + 1 * (factor ** 2 == n)
...
def make_averaged(original_function, samples_count=1000):
    ...
    def averager(*args):
        ...
        for _ in range(samples_count):
            sum_function += original_function(*args)
        return sum_function / samples_count
    return averager
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_score = 0
    for i in range(1, 11):
        exp = make_averaged(roll_dice, samples_count)(i, dice)
        if exp > max_score:
            max_score = exp
            max_roll = i
    return max_roll
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    return 0 if sus_update(0, score, opponent_score, six_sided) - score >= threshold else num_rolls
