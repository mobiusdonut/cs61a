...
def interleaved_sum(n, odd_func, even_func):
    ...
    True
    """
    def interleave_from(k):
        if k > n:
            return 0
        if k == n:
            return odd_func(k)
        else:
            return odd_func(k) + even_func(k + 1) + interleave_from(k + 2)
    return interleave_from(1)
...
def count_coins(total):
    ...
    True
    """
    def count_smaller(total, largest):
        if total == 0:
            return 1
        elif total <= 0 or largest == None:
            return 0
        else:
            return count_smaller(total, next_smaller_coin(largest)) + count_smaller(total - largest, largest)
    return count_smaller(total, 25)
