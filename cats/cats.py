...
def about(subject):
    ...
    def keyword_lookup(p):
        p_clean = split(lower(remove_punctuation(p)))
        if len(set(p_clean).intersection(set(subject))) > 0:
            return True
        else:
            return False
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        check = [1 if typed_words[i] == source_words[i] else 0 for i in range(min(len(typed_words), len(source_words)))]
        return sum(check) / len(typed_words) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        corr_word = min(word_list, key=lambda word: diff_function(typed_word, word, limit))
        if diff_function(typed_word, corr_word, limit) > limit:
            return typed_word
        else:
            return corr_word
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:
        ...
        return max(len(typed), len(source)) * (typed != source)
    else:
    ...
    while i <= len(typed):
        ...
        while j <= len(source):
            if i == 0 or j == 0:
                ...
                distance[(i, j)] = min(distance[(i - 1, j)] + 1, distance[(i, j - 1)] + 1, distance[(i - 1, j - 1)] + (typed[i - 1] != source[j - 1]))
            j += 1
        i += 1
    return distance[(len(typed), len(source))]
...
def time_per_word(words, timestamps_per_player):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    return words, [[player[i + 1] - player[i] for i in range(len(player[1:]))] for player in timestamps_per_player]
...
def fastest_words(match_object):
    ...
    fasts = [[] for _ in player_indices]
    for i in word_indices:
        min_player, min_time = 0, get_time(match_object, 0, i)
        for j in player_indices:
            if get_time(match_object, j, i) < min_time:
                min_player, min_time = j, get_time(match_object, j, i)
        fasts[min_player].append(get_word(match_object, i))
    return fasts
